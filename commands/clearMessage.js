module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!toDelete || toDelete.length === 0) return message.channel.send('No messages found to remove.');
    try {
      for (const m of toDelete) {
        await m.delete();
      }
      await message.channel.send(`Removed ${toDelete.length} messages.`).then(m => setTimeout(() => m.delete(), 3000));
    } catch (err) {
