...
Pre-commit hooks run `lint-staged` (ESLint --fix + Prettier on staged files) automatically unless your commit message begins with `wip:`—those commits intentionally skip the checks so you can checkpoint work. Pre-push hook saat ini hanya menjalankan `pnpm build` (yang di dalamnya memanggil format:check, lint, test secara sequential); rencanakan upgrade berikutnya jika ingin menjalankan langkah-langkah itu secara eksplisit lagi. Gunakan `HUSKY=0` hanya untuk emergency hotfix dan dokumentasikan alasannya di PR.

## Security & Configuration Tips

Setiap kontributor wajib mengisi `.env` lokal dengan `TOKEN=...` dan menjaga file tersebut di luar git. Gunakan `pnpm validate:env` sebelum menjalankan bot di perangkat baru. `auto_pull.sh` yang disertakan didesain untuk Termux dan masih memakai `git reset --hard` pada branch `stable`; pastikan server tempat script ini dijalankan tidak menyimpan perubahan lokal sensitif (token, patch manual) karena akan ditimpa. GitHub Actions workflow saat ini hanya menjalankan `pnpm build` (tanpa langkah format/lint/test terpisah); pembaruan tambahan dibutuhkan bila ingin hasil format/lint/test terlihat eksplisit di UI CI. Hindari mengaktifkan fitur self-bot di lingkungan produksi karena tetap melanggar Ketentuan Layanan Discord.
