...
Use Node.js 20.11+ to guarantee dukungan `global.fetch` dan API Discord terbaru. Install dependencies with `pnpm install` to respect the checked-in lockfile. Use `pnpm dev` (tsx watch) for live-reloading TypeScript development. Run `pnpm build` to emit JavaScript into `dist/`; build automatically runs `prebuild` first (`pnpm format:check && pnpm lint && pnpm test`) sehingga satu perintah sudah mencakup lint/test. `pnpm start` menjalankan `prestart` (build) sebelum `node dist/index.js`. Jalankan `pnpm test` (ts-jest) saat menambah command/event baru, `pnpm validate:env` untuk mengecek konfigurasi `.env`, dan `pnpm format` / `pnpm format:check` agar gaya konsisten. Restart `pnpm dev` watcher setiap menambah file baru supaya loader dinamis bisa memuatnya.
...
Pre-commit hooks run `lint-staged` (ESLint --fix + Prettier on staged files) automatically unless your commit message begins with `wip:`—those commits intentionally skip the checks so you can checkpoint work. Pre-push hook saat ini hanya menjalankan `pnpm build` karena script build sudah memicu lint/test melalui `prebuild`. Gunakan `HUSKY=0` hanya untuk emergency hotfix dan dokumentasikan alasannya di PR.

## Security & Configuration Tips

Setiap kontributor wajib mengisi `.env` lokal dengan `TOKEN=...` dan menjaga file tersebut di luar git. Gunakan `pnpm validate:env` sebelum menjalankan bot di perangkat baru. `auto_pull.sh` yang disertakan didesain untuk Termux dan masih memakai `git reset --hard` pada branch `stable`; pastikan server tempat script ini dijalankan tidak menyimpan perubahan lokal sensitif (token, patch manual) karena akan ditimpa. Hindari mengaktifkan fitur self-bot di lingkungan produksi karena tetap melanggar Ketentuan Layanan Discord.
