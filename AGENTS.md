...
Setiap kontributor wajib mengisi `.env` lokal dengan `TOKEN=...` dan menjaga file tersebut di luar git. Gunakan `pnpm validate:env` sebelum menjalankan bot di perangkat baru (opsional `-- --env-file .env.example --json` untuk memvalidasi file lain). `auto_pull.sh` sekarang melakukan fast-forward merge, menolak working tree kotor, menjalankan `pnpm install` + `pnpm build`, lalu merestart PM2 (`AUTO_PULL_PM2_ENTRY`, default `dist/index.js`); tetap pastikan branch `stable` punya artefak `dist/` siap pakai sebelum men-deploy. GitHub Actions workflow menjalankan tahapan Validate Env → Format → Lint → Test → Compile secara eksplisit sehingga kegagalan cepat terlihat. Hindari mengaktifkan fitur self-bot di lingkungan produksi karena tetap melanggar Ketentuan Layanan Discord.
